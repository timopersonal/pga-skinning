...
void VulkanglTFModel::updateJoints(VulkanglTFModel::Node *node)
{
	if (node->skin > -1)
	{
		...
		jointMatrices = std::vector<glm::mat4>(numJoints);
...
void VulkanExample::loadglTFFile(std::string filename)
{
	...
	for (double time_passed = 0.0; time_passed <= glTFModel.animations[glTFModel.activeAnimation].end; time_passed += time_step)
	{
		...
		for (uint32_t i = 0; i < test_vertices.size(); ++i)
		{
			...
			glm::mat4 blend_matrix =
				vtx.jointWeights.x * glTFModel.jointMatrices[vtx.jointIndices.x] +
				vtx.jointWeights.y * glTFModel.jointMatrices[vtx.jointIndices.y] +
				vtx.jointWeights.z * glTFModel.jointMatrices[vtx.jointIndices.z] +
				vtx.jointWeights.w * glTFModel.jointMatrices[vtx.jointIndices.w];
			glm_vtx = blend_matrix * glm_vtx;
			vtx.pos.z = glm_vtx.x;
			vtx.pos.x = glm_vtx.y;
			vtx.pos.y = glm_vtx.z;
			test_vertices[i] = vtx;
	...
	std::ofstream output_file("stats_lbs_" + model_name + ".txt");
...
void VulkanExample::preparePipelines()
{
	...
	const std::array<VkPipelineShaderStageCreateInfo, 2> shaderStages = {
		loadShader(getShadersPath() + "lbs/skinnedmodel.vert.spv", VK_SHADER_STAGE_VERTEX_BIT),
		loadShader(getShadersPath() + "lbs/skinnedmodel.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT)};
